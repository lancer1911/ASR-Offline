"""
launcher.py — Lancer1911 ASR Offline .app entry point for py2app

The .app itself is intentionally lightweight. It finds an external Python
environment that already has ASR dependencies installed, then runs main.py
from the app Resources directory.
"""
import os
import sys
import subprocess
from pathlib import Path

APP_NAME = "Lancer1911 ASR Offline"
LOG_NAME = "ASROffline.log"


def _quote_for_applescript(s: str) -> str:
    return s.replace('\\', '\\\\').replace('"', '\\"')


def alert(title: str, msg: str) -> None:
    try:
        script = (
            f'display alert "{_quote_for_applescript(title)}" '
            f'message "{_quote_for_applescript(msg)}" as critical'
        )
        subprocess.run(["osascript", "-e", script], check=False)
    except Exception:
        pass


def find_python() -> str | None:
    """Find a Python executable likely to contain the ASR runtime packages."""
    candidates = [
        Path.home() / "asr-offline-env" / "bin" / "python3",
        Path.home() / "asr_offline_env" / "bin" / "python3",
        Path.home() / "asr-env" / "bin" / "python3",
        Path.home() / "asr-env" / "bin" / "python",
    ]

    # Common project-local virtual environments when running from source.
    here = Path(__file__).resolve().parent
    candidates += [
        here / ".venv" / "bin" / "python3",
        here / "venv" / "bin" / "python3",
        here / "venv_build" / "bin" / "python3",
    ]

    # pyenv versions, newest name first.
    pyenv_root = Path.home() / ".pyenv" / "versions"
    if pyenv_root.exists():
        for version_dir in sorted(pyenv_root.iterdir(), reverse=True):
            candidates.append(version_dir / "bin" / "python3")

    candidates += [
        Path("/opt/homebrew/bin/python3"),
        Path("/usr/local/bin/python3"),
        Path("/usr/bin/python3"),
    ]

    for p in candidates:
        if p.exists() and os.access(p, os.X_OK):
            return str(p)
    return None


def find_main() -> str | None:
    """Find main.py in py2app Resources first, then next to this file."""
    resource_path = os.environ.get("RESOURCEPATH")
    if resource_path:
        main_py = Path(resource_path) / "main.py"
        if main_py.exists():
            return str(main_py)

    here = Path(__file__).resolve().parent
    main_py = here / "main.py"
    if main_py.exists():
        return str(main_py)
    return None


def main() -> None:
    python = find_python()
    if not python:
        alert(
            f"{APP_NAME} - Missing Python environment",
            "Cannot find a usable Python environment.\n\n"
            "Recommended setup:\n"
            "python3 -m venv ~/asr-offline-env\n"
            "source ~/asr-offline-env/bin/activate\n"
            "pip install -r requirements.txt\n\n"
            "Then launch the app again."
        )
        sys.exit(1)

    main_py = find_main()
    if not main_py:
        alert(f"{APP_NAME} - File missing", "Cannot find main.py. Please rebuild or reinstall the app.")
        sys.exit(1)

    work_dir = str(Path(main_py).parent)
    env = os.environ.copy()

    # py2app injects Python path variables that can contaminate the external
    # runtime environment. Remove them before launching the real app process.
    for key in [
        "PYTHONPATH", "PYTHONHOME", "PYTHONEXECUTABLE",
        "RESOURCEPATH", "EXECUTABLEPATH", "ARGVZERO",
    ]:
        env.pop(key, None)

    # Keep Homebrew and common user paths available when launched from Finder.
    path_parts = [
        "/opt/homebrew/bin", "/usr/local/bin", "/usr/bin", "/bin", "/usr/sbin", "/sbin",
        str(Path.home() / ".local" / "bin"),
    ]
    old_path = env.get("PATH", "")
    env["PATH"] = ":".join(path_parts + ([old_path] if old_path else []))

    log_path = Path.home() / "Library" / "Logs" / LOG_NAME
    log_path.parent.mkdir(parents=True, exist_ok=True)

    with open(log_path, "a", encoding="utf-8") as log:
        log.write(f"\n=== {APP_NAME} launch ===\n")
        log.write(f"Python: {python}\n")
        log.write(f"main.py: {main_py}\n")
        log.flush()
        proc = subprocess.run(
            [python, main_py],
            cwd=work_dir,
            env=env,
            stdout=log,
            stderr=log,
        )
    sys.exit(proc.returncode)


if __name__ == "__main__":
    main()
